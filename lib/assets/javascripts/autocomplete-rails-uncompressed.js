        ...
        change: function( event, ui ) {
            ...
            if (jQuery(this).attr('data-update-elements')) {
                ...
                for (var key in update_elements) {
                  jQuery.each(update_elements[key], function(index, element) {
                    ...
                    if (elem.is(':checkbox')) {
                        ...
                        elem.val(ui.item ? data[key] : "").trigger('change');
                    }
                  });
                }
            }
                ...
                jQuery.each(update_elements[key], function(index, element){
                  if(jQuery(element).attr("type") === "checkbox"){
                      ...
                      if((new_record && data[key] && data[key].indexOf('Create New') == -1) || !new_record){
                          jQuery(element).val(data[key]).trigger('change');
                      }else{
                          jQuery(element).val('').trigger('change');
                      }
                  }
                });
